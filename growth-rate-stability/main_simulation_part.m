...
% Resilience, Recovery
[resil_slow, recov_slow, resis_slow] = get_stability(t_slow, N_slow, K, method);
[resil_fast, recov_fast, resis_fast] = get_stability(t_fast, N_fast, K, method);
fprintf('\n\n\t\t Resilience\t Recovery')
fprintf('\nslow\t\t %1.2f\t\t %1.2f\t\t', resil_slow, recov_slow)
fprintf('\nfast\t\t %1.2f\t\t %1.2f', resil_fast, recov_fast)

% Resistance
[~, ~, resis_slow] = get_stability(t_slow, N_slow, K, method);
[~, ~, resis_fast] = get_stability(t_fast, N_fast, K, method);
[~, ~, resis_slow2] = get_stability(t_slow(idx_resi), N_slow(idx_resi), K, method);
[~, ~, resis_fast2] = get_stability(t_fast(idx_resi), N_fast(idx_resi), K, method);
fprintf('\n\n\t\tResistance')
...
[resil_slow, recov_slow] = get_stability(t_corr_slow, N_corr_slow, K);
[resil_fast, recov_fast] = get_stability(t_corr_fast, N_corr_fast, K);
...
resistance_fast_corr = N_fast_d(first_after_dist)/K;
resistance_slow_corr = y2/K;

fprintf('\n\n\t\t Re-scaled Resistance')
fprintf('\nfast\t\t %1.2f', resistance_fast_corr)
fprintf('\nslow\t\t %1.2f\n', resistance_slow_corr)  
...
% Resilience, Recovery
[resil_bad, recov_bad, resis_bad] = get_stability(t_bad, N_bad, K_bad, method);
[resil_good, recov_good, resis_good] = get_stability(t_good, N_good, K_good, method);
fprintf('\n\n\t\t Resilience\t Recovery')
fprintf('\nbad\t\t %1.2f\t\t %1.2f\t\t', resil_bad, recov_bad)
fprintf('\ngood\t\t %1.2f\t\t %1.2f', resil_good, recov_good)

% Resistance
[~, ~, resis_bad] = get_stability(t_bad, N_bad, K_bad, method);
[~, ~, resis_good] = get_stability(t_good, N_good, K_good, method);
[~, ~, resis_bad2] = get_stability(t_bad(idx_resi), N_bad(idx_resi), K_bad, method);
[~, ~, resis_good2] = get_stability(t_good(idx_resi), N_good(idx_resi), K_good, method);
fprintf('\n\n\t\tResistance')
fprintf('\nbad\t\t %1.2f\t', resis_bad2)
