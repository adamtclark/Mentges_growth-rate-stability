...
% S = readtable('/Users/am41xite/Documents/Data/Stability data/Hillebrand2020/all effect sizes.csv');
...
for c = 1:length(unique_cases)
    ...
    for r = 1:length(unique_responses)
        ...
        slopes = diff(ratio(post))./diff(day(post));
...
for c = 1:length(unique_cases)
    ...
    for r = 1:length(unique_responses)
        ...
        slopes = diff(ratio_resc)./diff(day_resc);
        if length(slopes)>=1
